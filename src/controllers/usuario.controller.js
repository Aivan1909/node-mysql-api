    ...
    if (password != null) {
      await bcrypt.hash(password, 5, async function (err, hashedPass) {
        ...
        if (hashedPass) {
          ...
          await connection.query(`INSERT INTO ${_TABLA} SET ?`, dataAdd)
            .then(async (response) => {
              const { insertId } = response
              const result = await connection.query(`SELECT * FROM ${_TABLA} WHERE id=?`, insertId)
...
const login = async (req, res) => {
  try {
    ...
    const connection = await getConnection()
    await connection.query(`SELECT * FROM ${_TABLA} WHERE email=?`, email)
      .then(user => {
...
const updateRegistro = async (req, res) => {
  try {
    ...
    const alianza = { nombre, enlace, imagen, estado, usuarioModificacion, fechaModificacion: new Date().toDateString() }
    const connection = await getConnection()
